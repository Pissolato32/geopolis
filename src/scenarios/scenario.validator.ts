...
export class ScenarioSchemaValidator {
  public validate(data: unknown): IScenarioValidationResult {
    const errors: IScenarioValidationError[] = [];
  ...
  private validateMetadata(metadata: unknown, errors: IScenarioValidationError[]): void {
  ...
  private validateWorldState(worldState: unknown, errors: IScenarioValidationError[]): void {
  ...
  private validateEntity(entity: unknown, path: string, errors: IScenarioValidationError[]): void {
    ...
    if (!Array.isArray(e['components'])) {
      ...
      for (let j = 0; j < e['components'].length; j++) {
        ...
        } else if (typeof (comp as Record<string, unknown>)['type'] !== 'string') {
          errors.push({ path: `${path}.components[${j}].type`, message: 'component.type is required and must be a string' });
        } else {
          const compTyped = comp as Record<string, unknown>;
  ...
  private validateProvince(province: unknown, path: string, errors: IScenarioValidationError[]): void {
    if (typeof province !== 'object' || province === null) {
      errors.push({ path, message: 'province must be a non-null object' });
      return;
    }

    const p = province as Record<string, unknown>;
  ...
  private validateRelation(relation: unknown, path: string, errors: IScenarioValidationError[]): void {
    ...
    if (!validRecognitions.includes(r['recognition'] as string)) {
      errors.push({ path: `${path}.recognition`, message: 'recognition must be one of: full, partial, unrecognized' });
    }
  }

  private validateEventTriggers(triggers: unknown, errors: IScenarioValidationError[]): void {
