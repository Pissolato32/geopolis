...
export function loadConfig(overrides?: Partial<IEngineConfig>): IEngineConfig {
  const args = parseArgv();
  const env = process.env;

  return {
    mode: (overrides?.mode ?? args['mode'] ?? env['GEOPOLIS_MODE'] ?? 'headless') as IEngineConfig['mode'],
...
export function loadSeedFromFile(path: string): unknown {
  if (!existsSync(path)) {
