...
import {
  ...
  IWorldStateMetadata,
  IWorldStateSnapshot,
} from '../interfaces/world-state.interface.js';
...
export class WorldState implements IWorldState {
  ...
  private readonly componentIndex: Map<ComponentType, Set<EntityId>> = new Map();
  private readonly relationGraphIndex: Map<EntityId, Map<EntityId, IComponent>> = new Map();
  ...
  private maybeCreateMirrorRelation(entityId: EntityId, component: IComponent): void {
    if (component.type !== 'diplomacy.relation') return;

    const compObj = component as unknown as Record<string, unknown>;
    const targetId = compObj['targetCountryId'];
    if (typeof targetId !== 'string') return;
    if (!this.entities.has(targetId as EntityId)) return;

    const targetEntity = this.entities.get(targetId as EntityId)!;
    if (targetEntity.hasComponent('diplomacy.relation' as ComponentType)) return;

    const mirrorComponent: IComponent = {
      type: 'diplomacy.relation' as ComponentType,
    ...
    this.indexComponent(targetId as EntityId, mirrorComponent.type);
  }
  ...
  dumpStateForAnalysis(options: IDenseStateDumpOptions): string {
    ...
    const intelComp = focalEntity?.getComponent<IComponent>('intel.agency');
    const intelObj = intelComp as unknown as Record<string, number> | undefined;
    const maxCapability = intelObj
      ? Math.max(
          intelObj['sigintCapability'] ?? 0,
          intelObj['humintCapability'] ?? 0,
          intelObj['osintCapability'] ?? 0,
          intelObj['imintCapability'] ?? 0,
          intelObj['cyberCapability'] ?? 0,
        )
      : 0;
    ...
    const visibleEntities = this.computeVisibleEntities(
      options.perspectiveEntityId,
      visibleRadius,
    );

    const lines: string[] = [
      `tick: ${this.currentTick as number}`,
    ...
    if (focalEntity) {
      ...
      for (const [type, comp] of focalEntity.getComponentMap()) {
        ...
        if (type === 'Demographic') {
          const dto = DenseFormatter.toDemographicViewDTO(comp as unknown as IDemographicComponent);
          lines.push(`    pop: "${dto.pop}"`);
          lines.push(`    trend: "${dto.trend}"`);
          lines.push(`    stability: "${dto.stability}"`);
        } else if (type === 'Economic') {
          const dto = DenseFormatter.toEconomicViewDTO(comp as unknown as IEconomicComponent);
          lines.push(`    gdp: "${dto.gdp}"`);
          lines.push(`    treasury: "${dto.treasury}"`);
          lines.push(`    inflation: "${dto.inflation}"`);
          lines.push(`    status: "${dto.status}"`);
        } else if (type === 'Military') {
          const dto = DenseFormatter.toMilitaryViewDTO(comp as unknown as IMilitaryComponent);
      ...
      if (visibleEntities.length > 0) {
        ...
        for (const vid of visibleEntities) {
          ...
          const relInfo = rel
            ? `affinity=${((rel as unknown as Record<string, number>)['affinity'] ?? 0).toFixed(2)}`
            : 'no_direct_relation';
  ...
  private computeVisibleEntities(
    ...
    if (!this.entities.has(focalId)) return [];

    const visited = new Set<EntityId>();
    const result: EntityId[] = [];
    const queue: Array<{ id: EntityId; depth: number }> = [{ id: focalId, depth: 0 }];
    visited.add(focalId);
  ...
  restoreFromSnapshot(snapshot: IWorldStateSnapshot): void {
    ...
    const rehydratedData = JSON.parse(
      JSON.stringify(snapshot.data, bigintJsonReplacer),
      bigintJsonReviver,
    ...
    for (const [entityId, componentData] of Object.entries(rehydratedData)) {
      ...
      for (const component of Object.values(componentData)) {
        components.push(component as IComponent);
      }
      this.createEntity(entityId as EntityId, components);
    }
  ...
  private getEntityOrThrow(id: EntityId): Entity {
    const entity = this.entities.get(id);
    if (!entity) {
  ...
  private indexComponent(entityId: EntityId, type: ComponentType): void {
    ...
    if (type === 'diplomacy.relation') {
      ...
      const comp = entity?.getComponent<IComponent>('diplomacy.relation');
      const compObj = comp as unknown as Record<string, unknown>;
      if (comp && compObj && typeof compObj['targetCountryId'] === 'string') {
        const targetId = compObj['targetCountryId'] as EntityId;
  ...
  private deindexComponent(entityId: EntityId, type: ComponentType): void {
